	...
	std::function<void*(uint8_t*, int, int, char)> createTexture)
{
	...
	NSBitmapImageRep *imageRep =
		[[NSBitmapImageRep alloc] initWithCGImage:[m_details->icon
	...
	unsigned char *rawData = (unsigned char *)calloc(
		height * width * 4, sizeof(unsigned char));
	...
	if (rawData) {
		...
		unsigned char *invData = (unsigned char *)calloc(
			height * width * 4, sizeof(unsigned char));
		if (invData) {
