...
static void checkErrNo()
{
	if (errno) {
		if (errno == EILSEQ)
			printf("DEBUG: errno = EILSEQ (illegal byte sequence), please "
			       "enable utf-8/wchar\n", errno);
		else
...
std::string DirectoryIterator::name() const
{
	...
	// m_entry->d_name is an array, not a pointer, so it cannot be nullptr.
	// If m_entry is valid, d_name is also valid.
...
void DirectoryIterator::readNextEntry()
{
	...
	while (m_impl->m_entry && (name == ".." || name == "."))
	{
		...
		name = m_impl->m_entry ? std::string(m_impl->m_entry->d_name) :
		       std::string();
	}
}
