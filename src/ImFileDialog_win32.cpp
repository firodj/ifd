...
FileInfoWin32::FileInfoWin32(const std::filesystem::path& path): FileInfoWin32()
{
	...
	DWORD_PTR ret = SHGetFileInfoW(pathW.c_str(), attrs, &m_details->fileInfo,
		sizeof(SHFILEINFOW), flags);
	/**
	If SHGetFileInfo returns an icon handle in the hIcon member of the
	SHFILEINFO structure pointed to by psfi, you are responsible for freeing it
